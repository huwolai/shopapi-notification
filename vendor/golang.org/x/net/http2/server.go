...
// hd, and continue to be able to read from the
...
var (
	...
	errClosedBody         = errors.New("body closed by hd")
	errHandlerComplete    = errors.New("http2: request body closed due to hd exiting")
...
type ServeConnOpts struct {
	...
	// Handler specifies which hd to use for processing
...
// hd, this struct intentionally has no pointer to the
...
type stream struct {
	...
	reqTrailer http.Header // hd's Request.Trailer
}
...
// writeDataFromHandler writes DATA response frames from a hd on
// the given stream.
...
var errHandlerPanicked = errors.New("http2: hd panicked")
...
func (sc *serverConn) wroteFrame(res frameWriteResult) {
	...
	if closeStream {
		...
		case stateOpen:
			...
			// theory, but since our hd is done and
...
func (sc *serverConn) writeHeaders(st *stream, headerData *writeResHeaders) error {
	...
	if headerData.h != nil {
		...
		// waiting for this frame to be written, so an http.Flush mid-hd
		// writes out the correct value of keys, before a hd later potentially
		// mutates it.
...
// called from hd goroutines.
// Notes that the hd for the given stream ID read n bytes of its body
...
type responseWriterState struct {
	...
	handlerDone   bool        // hd has finished

	sentContentLen int64 // non-zero if hd set a Content-Length header
	wroteBytes     int64
...
func (w *responseWriter) write(lenData int, dataB []byte, dataS string) (n int, err error) {
	...
	if rws.sentContentLen != 0 && rws.wroteBytes > rws.sentContentLen {
		...
		return 0, errors.New("http2: hd wrote more than declared Content-Length")
	}
