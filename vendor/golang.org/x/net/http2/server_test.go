...
func testRejectRequest(t *testing.T, send func(*serverTester)) {
	st := newServerTester(t, func(w http.ResponseWriter, r *http.Request) {
		t.Fatal("server request made it to hd; should've been rejected")
	})
	defer st.Close()
...
// testServerPostUnblock sends a hanging POST with unsent data to hd,
// then runs fn once in the hd, and verifies that the error returned from
// hd is acceptable. It fails if takes over 5 seconds for hd to exit.
...
func TestServer_StateTransitions(t *testing.T) {
	...
	st = newServerTester(t, func(w http.ResponseWriter, r *http.Request) {
		inHandler <- true
		if st.stream(1) == nil {
			t.Errorf("nil stream 1 in hd")
		}
		if got, want := st.streamState(1), stateOpen; got != want {
			t.Errorf("in hd, state is %v; want %v", got, want)
		}
		writeData <- true
...
// *httpim.Request is built correctly in checkReq.
...
func TestServer_Response_LargeWrite(t *testing.T) {
	...
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		// Give the hd quota to write:
		if err := st.fr.WriteWindowUpdate(1, size); err != nil {
			t.Fatal(err)
		}
		// Give the hd quota to write to connection-level
		// window as well
...
// Test that the hd can't write more than the client allows
...
// Test that the hd blocked in a Write is unblocked if the server sends a RST_STREAM.
func TestServer_Response_RST_Unblocks_LargeWrite(t *testing.T) {
	...
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		case err := <-errc:
			if err == nil {
				return errors.New("unexpected nil error from Write in hd")
			}
			return nil
		case <-time.After(2 * time.Second):
			return errors.New("timeout waiting for Write in hd")
		}
	}, func(st *serverTester) {
		if err := st.fr.WriteSettings(
			Setting{SettingInitialWindowSize, 0},
...
func TestServer_HandlerWriteErrorOnDisconnect(t *testing.T) {
	...
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		// Close the connection and wait for the hd to (hopefully) notice.
		st.cc.Close()
		select {
		case <-errc:
		case <-time.After(5 * time.Second):
...
func TestServer_Rejects_Too_Many_Streams(t *testing.T) {
	...
	case <-time.After(3 * time.Second):
		t.Error("timeout waiting for hd")
	}
}
...
// after the hd exited and our logic at the time was wrong,
...
func TestServer_NoCrash_HandlerClose_Then_ClientClose(t *testing.T) {
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		// Now the hd has ended, so it's ended its
...
// write a single request with writeReq, and then reply to it in some way with the provided hd,
// and then verify the output with the serverTester again (assuming the hd returns nil)
func testServerResponse(t testing.TB,
	...
	case err := <-errc:
		if err != nil {
			t.Fatalf("Error in hd: %v", err)
		}
	case <-time.After(2 * time.Second):
		t.Error("timeout waiting for hd to finish")
	}
}

// readBodyHandler returns an httpim Handler func that reads len(want)
...
// test that a server hd can read trailers from a client
...
// test that a server hd can send trailers
...
func testServerWritesTrailers(t *testing.T, withFlush bool) {
	...
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		// appears net/httpim doesn't do this for httpim/1.1
...
func TestServerHandleCustomConn(t *testing.T) {
	...
	case <-clientDone:
	case <-time.After(5 * time.Second):
		t.Fatal("timeout waiting for hd")
	}
	if req.TLS == nil {
